...
export function leerEstadoMock(seniaId: number): EstadoMock | undefined {
  return leerAlmacen()[String(seniaId)]
}

export function guardarEstadoMock(seniaId: number, estado: EstadoMock): void {
  const almacen = leerAlmacen()
  almacen[String(seniaId)] = estado
