...
| DocumentacionCliente | M:N de `Cliente.documentaciones` | `documentacion_cliente` |
...
- **`Senia.importe`, `estado`, `fechaVencimiento` y `pago`** no están en el diagrama,
  pero los exige el flujo de pago de la seña (ver `api-senias.md`). Conviene
  actualizar el `.drawio` para reflejarlos.
- **`Pago`** es una entidad nueva, por la misma razón. Guarda solo los últimos cuatro
